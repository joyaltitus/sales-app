...
import { useObjectionLogs } from '../../lib/objections-data'
...
export function ObjectionHistory({ contactId }: { contactId: string }) {
  ...
  const { items, loading, error } = useObjectionLogs(activeClient?.id ?? null, contactId)
  ...
  return (
    <section className="border-t border-border pt-4" aria-labelledby="objection-history-title">
      ...
      {error && <p className="mt-2 text-2xs font-semibold text-danger">Couldn’t load history: {error}</p>}
      {!loading && !error && items.length === 0 && <p className="mt-3 text-xs text-fg-subtle">No objections logged yet.</p>}
            ...
            <li key={item.id} className="relative flex gap-3 pb-4 last:pb-0">
              ...
              <div className="min-w-0 flex-1 pt-0.5">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-fg">{item.taxonomyLabel}</span>
                ...
                <p className="mt-1 text-2xs text-fg-subtle">{formatWhen(item.occurred_at)}{item.actorName ? ` · ${item.actorName}` : ''}</p>
              </div>
            </li>
          )
        })}
