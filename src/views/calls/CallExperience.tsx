...
export function CallExperience({ person, phone, dealValue, stage, onClose, onComplete }: { person: string; phone: string; dealValue: number; stage: string; onClose: () => void; onComplete: (outcome: CallOutcomePreview, callbackAt?: string) => void }) {
  ...
  return (
    <Sheet open onClose={onClose} title={step === 'brief' ? '15-second deal brief' : step === 'calling' ? `Calling ${person}` : 'How did it go?'}>
      ...
      {step === 'outcome' && <div>
        <div className="rounded-lg bg-surface-sunk p-3"><p className="label-caps">Call complete · 6m 12s</p><p className="mt-1 text-sm font-semibold text-fg">{person} · ₹{dealValue.toLocaleString('en-IN')}</p></div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <button onClick={() => finish('closed')} className="col-span-2 flex min-h-14 items-center justify-center gap-2 rounded-lg border border-success bg-success-subtle text-sm font-semibold text-success"><PartyPopper aria-hidden size={17} /> Closed · ₹{dealValue.toLocaleString('en-IN')}</button>
