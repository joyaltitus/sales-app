...
export function DealProbability({ probability, person = 'this deal' }: { probability: number; person?: string }) {
  ...
  return <><button onClick={(event) => { event.stopPropagation(); setOpen(true) }} className={['inline-flex min-h-8 shrink-0 items-center gap-1 rounded-pill border px-2 text-2xs font-semibold', tone].join(' ')} aria-label={`${probability}% stage-based deal signal for ${person}`} title="Preview stage-based signal — tap for the basis"><Gauge aria-hidden size={11} />{probability}%</button>{open && <Suspense fallback={null}><ProbabilityExplanation open probability={probability} person={person} onClose={() => setOpen(false)} /></Suspense>}</>
}
